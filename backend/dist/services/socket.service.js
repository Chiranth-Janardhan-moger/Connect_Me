...
exports.notifyTripEnded = exports.getIO = exports.initializeSocket = void 0;
const socket_io_1 = require("socket.io");
const bus_model_1 = require("../models/bus.model");
const location_util_1 = require("../utils/location.util");
const bus_repository_1 = __importDefault(require("../api/repositories/bus.repository"));
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('student:join', async ({ busId, studentId }) => {
            ...
            try {
                ...
                const bus = await bus_repository_1.default.findByRouteNumber(busId);
                if (!bus) {
                ...
                if (bus.tripStatus === bus_model_1.TripStatus.ON_ROUTE && bus.currentLat && bus.currentLon) {
        ...
        socket.on('driver:location-update', async ({ busId, latitude, longitude }) => {
            ...
            try {
                ...
                const bus = await bus_repository_1.default.findByRouteNumber(busId);
                if (!bus || bus.tripStatus !== bus_model_1.TripStatus.ON_ROUTE) {
                    // If trip ended, notify driver to stop sending updates
                    if (bus && bus.tripStatus === bus_model_1.TripStatus.NOT_STARTED) {
                ...
                const route = await bus_repository_1.default.findRouteByNumber(bus.routeNumber);
                if (route && route.stops && route.stops.length > 0) {
                    const lastStop = route.stops[route.stops.length - 1];
                    const distance = (0, location_util_1.calculateDistance)(latitude, longitude, lastStop.lat, lastStop.lon);
                ...
                await bus_repository_1.default.save(bus);
            }
            catch (error) {
        ...
        socket.on('chat:join', ({ routeNumber }, callback) => {
            if (!routeNumber) {
                if (callback)
                    callback({ success: false, error: 'Route number required' });
                return;
            }
...
const notifyTripEnded = (routeNumber) => {
    if (io) {
...
exports.notifyTripEnded = notifyTripEnded;
